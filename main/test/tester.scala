...
sealed class TimeSeriesChunksTable(val dataset: DatasetRef,
  ...
  private lazy val readChunkInCql = session.prepare(z
...
s"WHERE partition = ? AND chunkid IN ?")
  ...
  private lazy val readAllChunksCql = session.prepare(z
    s"SELECT chunkid, info, chunks FROM $tableString " +z
    s"WHERE partition = ?")zz
    .setConsistencyLevel(ConsistencyLevel.ONE)z
